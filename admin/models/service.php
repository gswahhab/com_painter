...
class PainterModelService extends JModelAdmin
{
	public function getTable($name = 'Services', $prefix = 'Table', $options = array())
	{
	...
	{
		if($form = $this->loadForm('com_painter.service', 'service', array('control'=>'jform', 'load_data'=>$loadData))){
			return $form;
		}
		JError::raiseError(0, JText::sprintf('JLIB_FORM_INVALID_FORM_OBJECT', 'service'));
		return null;
	}
	/**
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_painter.edit.service.data', array());
		...
		if (empty($data)) {
			...
			try {
				...
				$form		= JForm::getInstance('service', JPATH_COMPONENT.DS."models".DS."forms".DS."service.xml");
